...
void push(int left, int right) { // push
	if (top == n * times)
...
void max_heapify(int* data, int start, int end) { // create max heapify
	int tmp;
	int dad = start;
	int son = dad * 2 + 1;
	while (son <= end) {
...
void printData(double time, int flag) { // print sorting time
	int i;
	if (flag == 1)
...
void selfCheck(int* data, int n, int flag) { // check if correctly sorted or not
	int i;
	for (i = 0; i < n - 1; i++) {
		if (data[i] > data[i + 1])
			break;
...
// ---------------------------------------------------------------------------
void __fastcall TForm1::Button1Click(TObject * Sender) { // start sorting
	...
	int i;
	Button1->Caption = "Loading..."; // program is running
	...
	for (i = 1; i <= times; i++) {
		...
		if (flag1 == true) { // SelectionSort
			if (data)
				free(data);
			data = new int[i * n];
			getData(data, org_data, i * n); // copydata
			t_begin = clock(); // start
		...
		if (flag4 == true) { // QuickSort with recursive
			if (data)
				free(data);
			data = new int[i * n];
			getData(data, org_data, i * n);
		...
		if (flag5 == true) { // QuickSort with non-recursive
			if (data)
				free(data);
			data = new int[i * n];
			getData(data, org_data, i * n);
		...
		if (flag6 == true) { // MergeSort with recursive
			if (data)
				free(data);
			data = new int[i * n];
			getData(data, org_data, i * n);
		...
		if (flag7 == true) { // MergeSort with nons-recursive
			if (data)
				free(data);
			data = new int[i * n];
			getData(data, org_data, i * n);
	...
	for (int j = 0; j < times; j++) { // create series in chart
		i = 0;
		if (flag1 == true) {
			if (j == 0)
	...
	Button1->Caption = "Run"; // Program complete
}
// ---------------------------------------------------------------------------
