...
// ---------------------------------------------------------------------------
void __fastcall TForm1::Button2Click(TObject *Sender) { // animatiosn
	PageControl1->ActivePage = TabSheet3;   //move to  "Tour in Animation" page
...
void __fastcall TForm1::StringGrid2DrawCell(TObject *Sender, int ACol, int ARow,
	...
	// if grid is " " then turn to white
	else
		StringGrid2->Canvas->Brush->Color =
			TColor RGB(StrToInt(Edit5->Text), StrToInt(Edit6->Text),
		StrToInt(Edit7->Text));
	// if grid is number then turn to the color user choosed
